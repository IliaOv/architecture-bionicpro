-- Источники данных сервиса отчётов (эмуляция).
--   crm.*        — CRM-система (OLTP): клиенты и их протезы.
--   telemetry.*  — "DB" датчиков: сырые показания телеметрии протезов.
-- Обе схемы живут в одной БД `sources`, чтобы не плодить контейнеры;
-- логически это два независимых источника, которые объединяет ETL.

CREATE SCHEMA IF NOT EXISTS crm;
CREATE SCHEMA IF NOT EXISTS telemetry;

-- --------------------------------------------------------------------------
-- CRM: клиенты и устройства
-- username совпадает с preferred_username в Keycloak — это ключ, по которому
-- сервис отчётов сопоставляет аутентифицированного пользователя с его данными.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Телеметрия: сырые показания датчиков (высокая частота).
-- Сидируем ~45 дней истории до "вчера", 24 замера в сутки на устройство,
-- чтобы витрина заполнилась при первом запуске DAG (watermark-backfill).
-- --------------------------------------------------------------------------
...
-- Пользователь для чтения источников из ClickHouse (табличная функция postgresql()).
-- В демо используем того же владельца БД; отдельный readonly-роль можно завести при желании.
