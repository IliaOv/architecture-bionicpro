"""Хранение готовых отчётов в объектном хранилище (S3 API / MinIO) и выдача
ссылок на CDN.

Теория (OLAP): витрина обновляется пакетно/по расписанию, поэтому между сменами
версии витрины (watermark) один и тот же отчёт неизменен. Его достаточно
посчитать из ClickHouse один раз, положить в S3 и раздавать через CDN —
повторные тяжёлые SELECT в OLAP исключаются.

Ключ объекта версионно-адресуемый (watermark в пути): новая версия витрины →
новый URL → старый CDN-кеш перестаёт быть востребованным.

HMAC в имени файла: бакет публично читается CDN, но чужой URL нельзя подобрать.
"""
