"""ETL сервиса отчётов BionicPRO.

Извлекает данные из CRM (клиенты, устройства) и из DB телеметрии, объединяет их,
агрегирует телеметрию в разрезе клиента и суток и формирует витрину в OLAP
(ClickHouse). Витрина `reports.user_daily_report` рассчитана на быстрый доступ
по пользователю; сервис API читает готовые строки без вычислений в реальном времени.

Логика загрузки — инкрементальная по watermark:
  * start = (последняя обработанная дата + 1) либо минимальная дата телеметрии;
  * end   = "вчера" (грузим только завершённые сутки);
  * каждый день в [start; end] грузится идемпотентно (DELETE + INSERT);
  * после успеха watermark сдвигается на end.
Первый запуск сам добэкфиллит всю доступную историю.
"""
...
# Реквизиты источника CRM/телеметрии (Postgres) для табличной функции postgresql().
